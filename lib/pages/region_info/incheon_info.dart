...
class IncheonInfo extends StatefulWidget {
  const IncheonInfo({Key? key}) : super(key: key);
...
class _IncheonInfoState extends State<IncheonInfo> {
  ...
  Future<void> fetchRegionInfo(String contentTypeId, String subCategory) async {
    final apiKey = 'xHIYywSVOCXTorWSMxYoMW92r1or16xp%2FtCpAviub7VzP26w68%2BB22HAnjI%2FR6DFfXvd%2BuTxmHUYabfyeti4sw%3D%3D'; // 여기에 API 키를 입력하세요
    String url =
        'http://apis.data.go.kr/B551011/KorService1/areaBasedList1?ServiceKey=$apiKey&areaCode=2&contentTypeId=$contentTypeId&arrange=D&numOfRows=50&pageNo=1&MobileOS=ETC&MobileApp=AppTest&_type=json';
          ...
          child: Column(
            ...
            children: [
              if (image != null && image.isNotEmpty)
                Image.network(image, width: 200, fit: BoxFit.cover),
              SizedBox(height: 10),
              Text(
                '상세 주소:',
