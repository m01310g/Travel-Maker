...
class IncheonInfo extends StatefulWidget {
  const IncheonInfo({Key? key});

  @override
  _IncheonInfoState createState() => _IncheonInfoState();
}

class _IncheonInfoState extends State<IncheonInfo> {
  ...
  Future<void> fetchRegionInfo(String contentTypeId, String subCategory) async {
    const apiKey = 'xHIYywSVOCXTorWSMxYoMW92r1or16xp%2FtCpAviub7VzP26w68%2BB22HAnjI%2FR6DFfXvd%2BuTxmHUYabfyeti4sw%3D%3D'; // 여기에 API 키를 입력하세요
    String url =
        'http://apis.data.go.kr/B551011/KorService1/areaBasedList1?ServiceKey=$apiKey&areaCode=2&contentTypeId=$contentTypeId&arrange=D&numOfRows=200&pageNo=1&MobileOS=ETC&MobileApp=AppTest&_type=json';
  ...
  Widget _buildCategoryChips() {
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: List.generate(categories.length, (index) => _buildCategoryChip(categories[index
        ], index)),
      ),
    );
  }
  ...
  Widget _buildCategoryChip(Map<String, String> categoryData, int index) {
    ...
    return GestureDetector(
      ...
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 12.0), // 좀 더 넓은 간격으로 조정
        child: Column(
          children: [
            // 카테고리 텍스트
            Text(
            ...
            AnimatedContainer(
              ...
              height: isSelected ? 3.0 : 0.0, // 선택된 경우 파란 선 높이
              width: 30.0, // 파란 선 너비
